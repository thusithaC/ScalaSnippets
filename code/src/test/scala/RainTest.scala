...
class RainfallTest extends FunSuite {
  ...
  var towns: Array[String] = Array("Rome", "London", "Paris", "NY", "Vancouver", "Sydney", "Bangkok", "Tokyo", "Beijing", "Lima", "Montevideo", "Caracas", "Madrid", "Berlin")

  private def assertFuzzyEquals(act: Double, exp: Double): Unit = {
    val inrange: Boolean = Math.abs(act - exp) <= 1e-2
    if (inrange == false) {
      println(
        "abs(actual - expected) must be <= 1e-2. Expected was " +
          exp +
          ", but got " +
          act)
    }
  ...
  test("means"){
    ...
    assertFuzzyEquals(Rain.mean("London", data), 51.199999999999996)
  }

  test("variances"){
    assertFuzzyEquals(Rain.variance("London", data), 57.42833333333374)
  }
